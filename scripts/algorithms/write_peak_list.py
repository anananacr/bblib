...
peak_list = {"nPeak": 0,"peakTotalIntensity":[],"peakXPosRaw": [],"peakYPosRaw": []}
file_name_list=[]
event_list=[]
files_lst = open("/asap3/petra3/gpfs/p09/2023/data/11019088/processed/rodria/lists/"+splitext(basename(sys.argv[1]))[0]+"-hits.lst", 'w')
...
for count, line in enumerate(stream):
    if reading_chunks:
        if line.startswith('End of peak list'):
            ...
            peak_list["nPeak"] += 1
            powder[int(round(peak_y_pos,0)), int(round(peak_x_pos,0))] += 1e0*intensity            
        elif line.split(': ')[0]=='Image filename':
            file_name = line.split(': ')[-1][:-1]
        elif line.split(': ')[0]=='Event':
            event=int(line.split(': //')[-1])
            f = h5py.File(file_name, 'r')
            shift_vertical_mm = float(f['shift_vertical_mm'][event])
...
f = h5py.File("/asap3/petra3/gpfs/p09/2023/data/11019088/processed/rodria/hits/"+splitext(basename(sys.argv[1]))[0]+'-agg_hit.h5', 'w')
...
f.create_dataset('/entry/data/nPeak', data=peaknPeak)
