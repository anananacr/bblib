...
def write_agg_file(file_label: str, file_index: int, n_hits: int):
    f = h5py.File(
        f"/asap3/petra3/gpfs/p09/2023/data/11019088/processed/rodria/hits/{file_label}-{file_index}-agg_hit.h5",
        "w",
    )
    f.create_dataset("/entry/data/powder", data=powder[: max_ss + 1, : max_fs + 1])
    f.create_dataset(
        "/entry/data/data", data=agg_hits[:n_hits, :, :], compression="gzip"
    )
    ...
    if reading_chunks and n_hits < max_num_hits:
        if line.startswith("End of peak list"):
            ...
            if peak_x_pos <= max_fs:
                ...
                peak_list["nPeak"] += 1
                powder[
                    int(round(peak_y_pos, 0)), int(round(peak_x_pos, 0))
                ] += intensity
