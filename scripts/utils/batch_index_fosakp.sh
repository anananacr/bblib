...
command="indexamajig -i ${ROOT}/lists/${INPUT}.lst --indexing=xgandalf,asdf -o  ${ROOT}/streams/${OUTPUT}.stream -g ${ROOT}/geoms/eiger500k_corrected_beam_centre_shift_fosakp_for_index.geom -o  ${ROOT}/streams/${OUTPUT}.stream --peaks=cxi --copy-header=/entry/data/shift_vertical_mm --copy-header=/entry/data/event_id --copy-header=/entry/data/file_id --copy-header=/entry/data/nPeaks --no-check-peaks --integration=rings-nograd-nocen --int-radius=3,4,6 --no-refine --no-retry --no-revalidate "
if [ -n "$CELL" ];
then
...
echo $command
#--xgandalf-no-deviation-from-provided-cell --tolerance=5,3.2,0.8,1.5
$command