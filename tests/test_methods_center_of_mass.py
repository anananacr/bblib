from bblib.methods import CenterOfMass
from bblib.models import PF8Info
import h5py
import numpy as np

config = {
    "plots_flag": False,
    "offset": {"x": 0, "y": 0},
    "peak_region": {"min": 60, "max": 180},
    "bragg_peaks_for_center_of_mass_calculation": 0,
    "pixels_for_mask_of_bragg_peaks": 2,
}

plots_info = {
    "filename": "test",
    "folder_name": "test_center_of_mass",
    "root_path": "/Users/minoru/Documents/ana/scripts/bblib/tests",
    "value_auto": False,
    "value_max": 100,
    "value_min": 1,
    "axis_lim_auto": True,
    "xlim_min": 0,
    "xlim_max": 0,
    "ylim_min": 0,
    "ylim_max": 0,
    "color_map": "viridis",
    "marker_size": 20,
}

PF8Config = PF8Info()


def test_type_output_from_center_of_mass_without_plot():
    PF8Config.set_geometry_from_file("example/simple.geom")
    center_of_mass_method = CenterOfMass(
        config=config, PF8Config=PF8Config, plots_info=plots_info
    )
    with h5py.File("example/thin_glowing_ring_center_600_200.h5", "r") as f:
        data = np.array(f["ring"][0])
    center_from_center_of_mass = center_of_mass_method(data=data)
    assert isinstance(center_from_center_of_mass, list)


def test_type_output_from_center_of_mass_with_plot():
    PF8Config.set_geometry_from_file("example/simple.geom")
    config["plots_flag"] = True
    center_of_mass_method = CenterOfMass(
        config=config, PF8Config=PF8Config, plots_info=plots_info
    )
    with h5py.File("example/thin_glowing_ring_center_600_200.h5", "r") as f:
        data = np.array(f["ring"][0])
    center_from_center_of_mass = center_of_mass_method(data=data)
    assert isinstance(center_from_center_of_mass, list)


def test_output_from_center_of_mass_without_plot():
    PF8Config.set_geometry_from_file("example/simple.geom")
    center_of_mass_method = CenterOfMass(
        config=config, PF8Config=PF8Config, plots_info=plots_info
    )
    with h5py.File("example/thin_glowing_ring_center_600_200.h5", "r") as f:
        data = np.array(f["ring"][0])
    center_from_center_of_mass = center_of_mass_method(data=data)
    assert center_from_center_of_mass == [600, 200]


def test_output_from_center_of_mass_with_plot():
    PF8Config.set_geometry_from_file("example/simple.geom")
    config["plots_flag"] = True
    center_of_mass_method = CenterOfMass(
        config=config, PF8Config=PF8Config, plots_info=plots_info
    )
    with h5py.File("example/thin_glowing_ring_center_600_200.h5", "r") as f:
        data = np.array(f["ring"][0])
    center_from_center_of_mass = center_of_mass_method(data=data)
    assert center_from_center_of_mass == [600, 200]
