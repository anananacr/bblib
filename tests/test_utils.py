...
def test_type_output_from_fwhm_map_min_from_projection_without_plot():
    ...
    center = get_fwhm_map_min_from_projection(
        lines=lines, label="", output_folder="", pixel_step=1, plots_flag=False
    )
...
def test_type_output_from_fwhm_map_min_from_projection_with_plot():
    ...
    center = get_fwhm_map_min_from_projection(
        lines=lines,
        label="test",
        output_folder="center_refinement/plots/test_utils",
        pixel_step=1,
        plots_flag=True,
    )
    assert isinstance(center, list)
